...
class FlowerBloomApp {
    ...
    initParticles() {
        ...
        for (let i = 0; i < count; i++) {
            this.particles.push(new GestureParticle(this.canvas.width, this.canvas.height));
        }
    }
    ...
    // ----- Flower Head (Sunflower facing camera) -----
    drawFlowerHead(cx, cy, bloom, windAngle, scale) {
        // Boost scale as it blooms
        ...
        const glowR = (80 + bloom * 120) * adjustedScale;
        if (bloom > 0.02) {
            const glow = ctx.createRadialGradient(0, 0, 0, 0, 0, glowR);
            glow.addColorStop(0, `rgba(255, 200, 50, ${0.4 * bloom})`);
            glow.addColorStop(0.5, `rgba(255, 150, 0, ${0.2 * bloom})`);
        ...
        const maxPetalLen = 80 * adjustedScale;
        const petalWidth = 16 * adjustedScale;
        
        // Two layers of petals radiating outward
        const numPetals = 16;
        ...
        if (bloom > 0.02) {
            ...
            for (let i = 0; i < numPetals; i++) {
                ...
                const flutter = this.noise.get(this.time * 1.2 + i, 3) * 0.05 * (1 + bloom);
                const finalAngle = baseAngle + flutter + windAngle * 0.1;
                
                const len = maxPetalLen * (0.3 + bloom * 0.7);
                const wid = petalWidth * (0.3 + bloom * 0.7);
                
                this.drawSunflowerPetal(ctx, finalAngle, len, wid, bloom);
            }
            
            // Front layer petals (offset by half a petal)
            for (let i = 0; i < numPetals; i++) {
                const baseAngle = ((i + 0.5) / numPetals) * Math.PI * 2;
                const flutter = this.noise.get(this.time * 1.4 + i, 4) * 0.05 * (1 + bloom);
                const finalAngle = baseAngle + flutter + windAngle * 0.1;
                
                const len = maxPetalLen * 0.9 * (0.3 + bloom * 0.7);
                const wid = petalWidth * 0.9 * (0.3 + bloom * 0.7);
        ...
        // Draw Center Disc
        const discRadius = 26 * adjustedScale * (0.2 + bloom * 0.8);
        
        if (bloom > 0.02) {
            ctx.shadowBlur = 8;
            ctx.shadowColor = 'rgba(0,0,0,0.6)';
            
            const centerGrad = ctx.createRadialGradient(0, 0, 0, 0, 0, discRadius);
            centerGrad.addColorStop(0, '#4a2f14');
            centerGrad.addColorStop(0.7, '#2a1705');
            centerGrad.addColorStop(1, '#150a01');
            ...
            // Seeds texture
            ctx.shadowBlur = 0;
            ctx.fillStyle = `rgba(255, 200, 100, ${0.4 * bloom})`;
            const seedLayers = 6;
            for (let ring = 1; ring <= seedLayers; ring++) {
                const r = (discRadius * ring) / seedLayers;
                const seedsInRing = ring * 7;
                for (let i = 0; i < seedsInRing; i++) {
                    const a = (i / seedsInRing) * Math.PI * 2 + this.time * 0.3;
                    const sx = Math.cos(a) * r * 0.85;
                    const sy = Math.sin(a) * r * 0.85;
                    ctx.beginPath();
                    ctx.arc(sx, sy, 1.2 * adjustedScale * bloom, 0, Math.PI * 2);
                    ctx.fill();
                }
            }
        }
    ...
    drawSunflowerPetal(ctx, angle, length, width, bloom) {
        ctx.save();
        ctx.rotate(angle);

        // Sunflower yellow/orange gradient
        const grad = ctx.createLinearGradient(0, 0, 0, -length);
        grad.addColorStop(0, `hsla(38, 95%, 45%, 0.95)`); // dark orange-yellow base
        grad.addColorStop(0.4, `hsla(48, 95%, 55%, 0.9)`); // rich yellow
        grad.addColorStop(1, `hsla(55, 95%, 65%, 0.95)`); // lighter yellow tip

        ctx.fillStyle = grad;
        ctx.shadowBlur = 6 + bloom * 8;
        ctx.shadowColor = `hsla(45, 100%, 50%, ${0.15 + bloom * 0.25})`;

        // Petal shape (ellipse pointed at tip)
        ctx.beginPath();
        ctx.moveTo(0, 0);
        ctx.bezierCurveTo(
            -width * 1.5, -length * 0.3,
            -width * 1.1, -length * 0.8,
            0, -length
        );
        ctx.bezierCurveTo(
            width * 1.1, -length * 0.8,
        ...
        // Subtle petal vein
        ctx.shadowBlur = 0;
        ctx.strokeStyle = `hsla(35, 90%, 40%, 0.3)`;
        ctx.lineWidth = 1;
        ctx.beginPath();
        ctx.moveTo(0, 0);
...
// BOOT
// =============================================================
let flowerAppInstance = null;
function initSceneFlower() {
    if (!flowerAppInstance) {
        flowerAppInstance = new FlowerBloomApp();
    }
}
